...
impl RequiresPython {
    ...
    pub fn to_marker_tree(&self) -> MarkerTree {
        let mut marker = self.range.lower().to_marker_tree(true);
        marker.and(self.range.upper().to_marker_tree(false));
        marker
    }
...
impl RequiresPythonBound {
    ...
    fn to_marker_tree(&self, greater: bool) -> MarkerTree {
        let (op, version) = match self.0 {
            ...
            Bound::Excluded(ref version) => {
                ...
                (op, version.clone().without_local())
            }
